...
async function syncAdminUsers(connection) {
  ...
  await deactivateLegacyAdmins(connection, configuredUsername);
}

async function deactivateLegacyAdmins(connection, activeUsername) {
...
async function migrateExistingSchema(connection) {
  ...
  if (!productColumns.length) {
    await connection.query("ALTER TABLE products ADD COLUMN store_section VARCHAR(40) NOT NULL DEFAULT 'panels' AFTER id");
  }

  const [purchaseOptionColumns] = await connection.query(
    `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'products' AND COLUMN_NAME = 'purchase_options'`
  );
  if (!purchaseOptionColumns.length) {
