(function () {
  const warningText = "Pagina protegida por YOJHAN-STORE. No esta permitido copiar ni inspeccionar.";
  ...
  document.addEventListener("contextmenu", (event) => {
    protectEvent(event, "Accion bloqueada por seguridad de YOJHAN-STORE.");
  }, true);
  ...
  document.addEventListener("selectstart", (event) => protectEvent(event), true);
  ...
  document.addEventListener("keydown", (event) => {
    ...
    const blocksMacDevtools = event.metaKey && event.altKey && blockedKeys.has(key);
    if (!blocksDevtools && !blocksSourceOrSave && !blocksMacDevtools) return;
    protectEvent(event, "Panel protegido. No esta permitido inspeccionar ni copiar la pagina.");
  }, true);

  window.addEventListener("beforeprint", (event) => {
    protectEvent(event, "Impresion bloqueada por seguridad de YOJHAN-STORE.");
  }, true);
})();
