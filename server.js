...
const JWT_SECRET = process.env.JWT_SECRET || "yojhan_store_dev_secret";
const IS_PRODUCTION = process.env.NODE_ENV === "production";
const SESSION_COOKIE = "yojhan_store_admin";
const CUSTOMER_COOKIE = "yojhan_store_customer";
const BRAND_NAME = "YOJHAN-STORE";
const BRAND_LOGO = "assets/yojhan-logo.png";
const ADMIN_PANEL_PATH = "/yojhan-control-web729hf";
const BLOCKED_ADMIN_PATHS = ["/admin", "/admin.html", "/jhon-panel-sxnsi", "/control-sxnsi-jhon-9f7k2q"];
const WHATSAPP_GROUP = "https://wa.me/18493753965";
const WHATSAPP_NUMBER = "18493753965";
const OWNER_WHATSAPP_NUMBER = "18493753965";
const YOAN_WHATSAPP_NUMBER = "18493753965";
const ADMIN_EMAIL = process.env.ADMIN_EMAIL || "";
const EMAIL_FROM = process.env.EMAIL_FROM || (ADMIN_EMAIL ? `${BRAND_NAME} <${ADMIN_EMAIL}>` : BRAND_NAME);
...
const ADMIN_DEFAULT_USERNAME = "YOJHAN";
const ADMIN_DEFAULT_PASSWORD_HASH = "$2a$10$CcAugrNPmAOWPV.gXLSOI.4RFDfiXhh8AkoafafP0993Wf90OpZci";
const LEGACY_ADMIN_USERNAMES = ["Jhon", "Reyes23", "Garcita9"];
...
const CATALOG_SECTIONS = new Set(["panels"]);
const DEFAULT_PRODUCTS = [
  {
    key: "bypass",
    aliases: ["BYPASS", "Bypass"],
    name: "BYPASS",
    storeSection: "panels",
    category: "Panel",
    description: "Bypass premium para Free Fire con compra directa por WhatsApp y soporte guiado.",
    imageUrl: "assets/yojhan-product-bypass.png",
    oldPrice: null,
    price: 5,
    badge: "BYPASS",
    options: [
      { label: "7 dias", price: "5 USD" },
      { label: "15 dias", price: "10 USD" },
      { label: "30 dias", price: "20 USD" },
      { label: "60 dias", price: "40 USD" }
    ]
  },
  {
    key: "panel-esp",
    aliases: ["PANEL ESP", "Panel ESP", "Lineas", "Hologramas"],
    name: "PANEL ESP",
    storeSection: "panels",
    category: "Lineas / Hologramas",
    description: "Panel ESP solo lineas o hologramas. Ligero, directo y con soporte por WhatsApp.",
    imageUrl: "assets/yojhan-product-panel-esp.png",
    oldPrice: null,
    price: 5,
    badge: "ESP",
    options: [{ label: "10 dias", price: "5 USD" }]
  },
  {
    key: "panel-basico",
    aliases: ["PANEL BASICO", "Panel basico", "Lineas Hologramas Aimbot"],
    name: "PANEL BASICO",
    storeSection: "panels",
    category: "Lineas + Hologramas + Aimbot",
    description: "Panel basico con lineas, hologramas y aimbot para Free Fire.",
    imageUrl: "assets/yojhan-product-panel-basico.png",
    oldPrice: null,
    price: 5,
    badge: "BASICO",
    options: [
      { label: "7 dias", price: "5 USD" },
      { label: "15 dias", price: "10 USD" },
      { label: "30 dias", price: "15 USD" }
    ]
  },
  {
    key: "panel-supreme",
    aliases: ["PANEL SUPREME", "Panel Supreme"],
    name: "PANEL SUPREME",
    storeSection: "panels",
    category: "Supreme",
    description: "Panel Supreme con funciones premium y opciones desde semanal hasta permanente.",
    imageUrl: "assets/yojhan-product-panel-supreme.png",
    oldPrice: null,
    price: 5,
    badge: "SUPREME",
    options: [
      { label: "7 dias", price: "5 USD" },
      { label: "15 dias", price: "15 USD" },
      { label: "30 dias", price: "20 USD" },
      { label: "Permanente", price: "50 USD" }
    ]
  },
  {
    key: "panel-extreme",
    aliases: ["PANEL EXTREME", "Panel Extreme", "Descaro total"],
    name: "PANEL EXTREME",
    storeSection: "panels",
    category: "Descaro total",
    description: "Panel Extreme descaro total para quienes quieren el paquete mas fuerte.",
    imageUrl: "assets/yojhan-product-panel-extreme.png",
    oldPrice: null,
    price: 12,
    badge: "EXTREME",
    options: [
      { label: "7 dias", price: "12 USD" },
      { label: "15 dias", price: "15 USD" },
      { label: "30 dias", price: "35 USD" },
      { label: "Permanente", price: "75 USD" }
    ]
  }
];
...
app.use((req, res, next) => {
  ...
  if (normalizedPathname !== pathname) {
    req.url = `${normalizedPathname}${queryString ? `?${queryString}` : ""}`;
  }
  next();
});
...
app.use([ADMIN_PANEL_PATH, ...BLOCKED_ADMIN_PATHS, "/api/admin", "/api/auth"], (_req, res, next) => {
  res.setHeader("Cache-Control", "no-store, max-age=0");
  res.setHeader("X-Robots-Tag", "noindex, nofollow, noarchive");
  next();
});
app.use([ADMIN_PANEL_PATH, ...BLOCKED_ADMIN_PATHS, "/api/admin", "/api/me"], blockAdminAreaIfNeeded);
...
async function ensureSchema() {
  const sectionColumns = await query(
    `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'products' AND COLUMN_NAME = 'store_section'`
  );
  if (!sectionColumns.length) {
    await query("ALTER TABLE products ADD COLUMN store_section VARCHAR(40) NOT NULL DEFAULT 'panels' AFTER id");
  }
  ...
  await query("ALTER TABLE analytics_events MODIFY event_type ENUM('page_view', 'whatsapp_click', 'buy_click') NOT NULL");
  await addIndexIfMissing("products", "idx_products_store_section", "INDEX idx_products_store_section (store_section, active, sort_order)");
...
function cleanProduct(body) {
  const storeSection = cleanLimited(body.storeSection || body.store_section, "panels", 40);
  return {
    name: cleanLimited(body.name, "", 180),
    storeSection: CATALOG_SECTIONS.has(storeSection) ? storeSection : "panels",
...
function productJson(product) {
  return {
    id: product.id,
    name: product.name,
    storeSection: product.store_section || "panels",
...
function paymentMethodsJson(productContext = {}) {
  const productLine = productContext.productName
    ...
    : "";
  const remitlyMessage = `Hola YOJHAN-STORE, vengo de la pagina y quiero comprar por WhatsApp.${productLine} Me ayudas con el proceso?`;
  return [
    {
      id: "transferencia",
...
function supportLinks(order) {
  const baseMessage = [
    `Hola, vengo de la pagina ${BRAND_NAME}.`,
  ...
  return [
    { label: "Soporte YOJHAN-STORE", phone: "+1 849 375 3965", url: buildWhatsappUrl(YOAN_WHATSAPP_NUMBER, baseMessage) },
    { label: "Dueño YOJHAN", phone: "+1 849 375 3965", url: buildWhatsappUrl(OWNER_WHATSAPP_NUMBER, baseMessage) }
  ];
}
...
function gmailOAuthConfig() {
  const user = cleanEmail(process.env.GMAIL_USER || ADMIN_EMAIL);
  ...
  const rawFrom = cleanText(process.env.EMAIL_FROM || process.env.GMAIL_FROM);
...
function formatGmailFrom(value, user) {
...
function safeEmailProviderStatus() {
  ...
  return {
    ...
    from: config.configured ? formatGmailFrom(config.from, config.user) : null,
...
function getEmailOAuth2Client() {
  ...
  if (!emailOAuth2Client) {
    emailOAuth2Client = new google.auth.OAuth2(config.clientId, config.clientSecret);
    emailOAuth2Client.setCredentials({ refresh_token: config.refreshToken });
  }
          ...
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:620px;background:#080b14;border:1px solid rgba(255,37,56,.45);border-radius:18px;overflow:hidden;box-shadow:0 22px 55px rgba(0,0,0,.35);">
            <tr>
              <td style="padding:24px 24px 12px;text-align:center;background:radial-gradient(circle at 50% 0%,rgba(255,37,56,.2),transparent 58%);">
                ...
                <div style="margin-top:9px;color:#ffffff;font-size:18px;font-weight:900;letter-spacing:.8px;">YOJHAN-STORE</div>
              </td>
            </tr>
            <tr>
...
function topupReceivedAdminEmail(customer, topup) {
  return genericStoreEmail({
    ...
    rows: [
      ...
      ["Metodo", topup.method],
      ["Monto", `${topup.amount.toFixed(2)} USD`],
      ["Precio producto", topup.price ? `${topup.price.toFixed(2)} USD` : ""],
...
function topupApprovedEmail(customer, topup, balance, adminNote) {
  return genericStoreEmail({
    ...
    subtitle: `Tu compra fue aprobada. Gracias por comprar en ${BRAND_NAME}.`,
    lines: [
      `Hola ${customer.name},`,
    ...
    rows: [
      ...
      ["Monto agregado", `${Number(topup.amount).toFixed(2)} USD`],
...
async function sendStoreEmail(input, subject, body) {
  ...
  const config = gmailOAuthConfig();
  if (!config.configured) {
    const reason = "Faltan GMAIL_USER, GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET o GOOGLE_REFRESH_TOKEN.";
    console.warn(`[email] ${reason} Guardando correo pendiente para ${email}: ${cleanSubject}`);
    await rememberEmail(email, cleanSubject, text, "pending", reason);
    return {
      sent: false,
      queued: true,
...
function purchaseReceiptEmail(customer, order, balanceAfter) {
  return genericStoreEmail({
    ...
    lines: [
      `Hola ${customer.name},`,
      `Gracias por comprar en ${BRAND_NAME}. Guarda este comprobante y tu PIN/KEY.`,
      `Para soporte, escribe al WhatsApp de ${BRAND_NAME} desde los botones de la pagina.`
    ],
    rows: [
      ...
      ["Precio", `${normalizeAmount(order.price).toFixed(2)} USD`],
      ["Bono agregado", `${normalizeAmount(order.rewardAmount).toFixed(2)} USD`],
      ["Saldo actual", `${normalizeAmount(balanceAfter).toFixed(2)} USD`],
...
function selectedProductPurchase(product, selectedOption) {
  ...
  if (options.length) {
    ...
    return {
      label: option.label,
      priceText: option.price || `${price} USD`,
      price
    };
  }
...
function defaultProductJson(product, index) {
  return {
    id: index + 1,
    name: product.name,
    storeSection: product.storeSection || "panels",
    category: product.category,
    description: product.description,
...
async function syncConfiguredAdmin() {
  ...
  await deactivateLegacyAdmins(configuredUsername);
}
...
async function deactivateLegacyAdmins(activeUsername) {
  for (const username of LEGACY_ADMIN_USERNAMES) {
    if (username === activeUsername) continue;
    await query(
      "UPDATE users SET active = 0, updated_at = NOW() WHERE role = 'admin' AND username = :username",
      { username }
    );
  }
}
...
async function upsertDefaultProduct(existingProduct, product, index) {
  ...
  if (existingProduct) {
    await query(
      `UPDATE products SET name = :name, store_section = :storeSection, category = :category, description = :description,
  ...
  await query(
    `INSERT INTO products (name, store_section, category, description, image_url, old_price, price, badge, purchase_options, active, sort_order)
     VALUES (:name, :storeSection, :category, :description, :imageUrl, :oldPrice, :price, :badge, :purchaseOptions, 1, :sortOrder)`,
    data
  );
}
...
app.post("/api/wallet/topups", customerAuth, asyncHandler(async (req, res) => {
  ...
  if (amount <= 0) return res.status(400).json({ error: "El monto de recarga debe ser mayor a 0 USD." });
...
app.post("/api/customer/purchase", customerAuth, asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (balance < purchase.price) {
      const error = new Error(`Saldo insuficiente. Tienes ${balance.toFixed(2)} USD y necesitas ${purchase.price.toFixed(2)} USD.`);
      error.statusCode = 400;
      throw error;
    }
...
app.get("/api/products", asyncHandler(async (req, res) => {
  const requestedSection = cleanLimited(req.query.section, "", 40);
  const section = CATALOG_SECTIONS.has(requestedSection) ? requestedSection : null;
  if (!isDatabaseReady()) {
    const products = DEFAULT_PRODUCTS.map(defaultProductJson)
      .filter((product) => !section || product.storeSection === section);
    return res.json(products);
  }
  const rows = section
    ? await query("SELECT * FROM products WHERE active = 1 AND store_section = :section ORDER BY sort_order ASC, id DESC", { section })
    : await query("SELECT * FROM products WHERE active = 1 ORDER BY store_section ASC, sort_order ASC, id DESC");
...
app.post("/api/admin/products", auth, adminOnly, asyncHandler(async (req, res) => {
  ...
  const result = await query(
    `INSERT INTO products (name, store_section, category, description, image_url, old_price, price, badge, purchase_options, active, sort_order)
     VALUES (:name, :storeSection, :category, :description, :imageUrl, :oldPrice, :price, :badge, :purchaseOptions, :active, :sortOrder)`,
    product
  );
...
app.post("/api/admin/customers/:id/balance", auth, adminOnly, asyncHandler(async (req, res) => {
  ...
  await sendStoreEmail(
    ...
    [
      `Hola ${customer.name},`,
      "",
      `Tu saldo fue actualizado por admin: ${amount > 0 ? "+" : ""}${amount.toFixed(2)} USD.`,
      `Nuevo saldo: ${balance.toFixed(2)} USD.`,
      `Nota: ${note}`
    ].join("\n")
  );
...
app.get("/api/admin/backup", auth, adminOnly, asyncHandler(async (_req, res) => {
  ...
  res.setHeader("Content-Disposition", `attachment; filename="yojhan-store-backup-${Date.now()}.json"`);
  res.json({
...
app.get(ADMIN_PANEL_PATH, (_req, res) => res.sendFile(path.join(__dirname, "admin.html")));
app.get("/cuentas.html", (_req, res) => res.redirect(302, "/"));
app.get(BLOCKED_ADMIN_PATHS, (_req, res) => {
